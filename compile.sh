...
C_SOURCE=`${INSTALL_DIR}/bin/compiler < ${SOURCE} | ${CPS_TRANSFORM} | ${INSTALL_DIR}/bin/lambda-lift | ${INSTALL_DIR}/bin/codegen`
if [[ -n "${DEBUG}" ]]; then
    C_SOURCE=`${INSTALL_DIR}/bin/compiler < ${SOURCE} | tee ${DEBUG_DIR}/symbol-table-output.lsk | ${CPS_TRANSFORM} | tee ${DEBUG_DIR}/cps-transform-output.lsk | ${INSTALL_DIR}/bin/lambda-lift | tee ${DEBUG_DIR}/lambda-lift-output.lsk | ${INSTALL_DIR}/bin/codegen`
fi
